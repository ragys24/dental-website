/* =============================================================
   ...
   Visual style: calm teal local-service guide with factual area context.
   Avoid fixed drive-time, financing, rating, or outcome promises.
   ============================================================= */
import Navbar from "@/components/Navbar";
import Footer from "@/components/Footer";
import { Link } from "wouter";
import { MapPin, Phone, MessageSquare, CheckCircle, Shield, ChevronRight } from "lucide-react";
...
export interface LocalAreaPageProps {
  ...
  driveTime?: string; // Retained for route compatibility; not displayed as an estimated travel promise.
  slug: string;
...
const FAQS = (city: string) => [
  ...
  },
  {
    ...
    a: `Yes. We accept Denti-Cal, most PPO insurance plans, and military insurance. The team can also discuss CareCredit, Cherry, and in-house financing options during your visit.`,
  },
  {
...
export default function LocalAreaPage({ city, state = "CA", distance, slug, nearbyLandmark, metaDescription, intro, zipCode }: LocalAreaPageProps) {
  const faqs = FAQS(city);
  const canonicalPath = slug.startsWith("dentist-near-") ? `/${slug}` : `/dentist-near-${slug}`;
  const defaultDescription = `Uplift Dental serves ${city}, ${state}${zipCode ? ` ${zipCode}` : ""} with general, cosmetic, and specialty dentistry${distance ? `, approximately ${distance} from Garden Grove` : ""}. Free consultations.`;
  const safeIntro = (intro || `Uplift Dental & Orthodontics proudly serves patients from ${city} with expert general dentistry, Invisalign®, dental implants, orthodontics, and oral surgery — all under one roof in nearby Garden Grove.`)
    .replace(/(?:\s*—\s*)?just\s+\d+\s+minutes(?:\s+away|\s+from\s+[^.]+)?/i, " conveniently located from nearby Garden Grove")
    .replace(/\s{2,}/g, " ");
  ...
  return (
    <>
      <PageSEO
        ...
        canonical={`https://upliftdental.com${canonicalPath}`}
      />
      <FAQSchema id={`ld-faq-${slug}`} faqs={faqs.map(({ q, a }) => ({ question: q, answer: a }))} />
          ...
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <h2 className="font-display text-3xl md:text-4xl font-bold mb-6 leading-tight" style={{ color: COLORS.tealDark }}>
                Coordinated Multi-Specialty Dental Care
              </h2>
              <p className="font-body text-gray-600 leading-relaxed mb-6">
                Patients from {city} can discuss general dentistry, orthodontics, and specialty-care needs with one Garden Grove practice. The team helps coordinate the next appropriate step for your family’s care.
              </p>
              ...
              <div className="grid grid-cols-2 gap-4">
                {[
                  { label: "Coordinated care", value: "One Team" },
                  { label: "Digital planning", value: "iTero & Trios" },
                  { label: "Coverage support", value: "Denti-Cal & PPO" },
                  { label: "Saturday hours", value: "3rd Sat." },
                ].map((stat) => (
            ...
            <div className="relative">
              <img
              src="https://d2xsxph8kpxj0f.cloudfront.net/310519663519418507/8XjTa97CZebFmBgqStQiLN/uplift-team-real_7d2676d0.jpg"
            ...
            />
              <div className="absolute -bottom-4 -left-4 bg-white rounded-xl shadow-lg p-4 border" style={{ borderColor: "oklch(0.88 0.04 192)" }}>
                <p className="font-body text-xs font-semibold text-gray-700">Garden Grove practice team</p>
                <p className="font-body text-xs text-gray-500">Serving nearby communities</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="py-16" style={{ background: `linear-gradient(135deg, ${COLORS.tealDark} 0%, ${COLORS.teal} 100%)` }}>
        <div className="container max-w-4xl mx-auto px-4 text-center">
          ...
          {nearbyLandmark && (
            <p className="font-body text-white/80 text-sm mb-8">{nearbyLandmark}</p>
          )}
